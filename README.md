...
The command rescans the local DBI Services and Medium exports, fetches the current Dev.to article list, downloads only snapshots that are not already present, and regenerates the manifest. Use `--refresh-devto` to replace all existing Dev.to snapshots with the latest API representation.
