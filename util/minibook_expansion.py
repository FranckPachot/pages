...
EXTRA_BOOKS = [
    ...
    {
        "slug": "document-data-modeling",
        "number": "23",
        "title": "Document Data Modeling",
        "subtitle": "JSONB, BSON, nesting, indexes, and consistency",
        "description": "Model document data deliberately across MongoDB and PostgreSQL, from aggregate boundaries and embedded arrays to indexing and update amplification.",
        "accent": "#4f7651",
        "topics": "MongoDB · JSONB · BSON · document model",
        "chapters": [
            chapter("A document is an aggregate boundary", "A useful document groups values that are read, validated, and changed together. Embedding is a transactional and locality decision, not simply a way to avoid schema design.", "Embed data with the same lifecycle and ownership.", "Reference independently growing or shared entities.", "Design from access and update patterns, not source-table shape."),
            chapter("Nesting carries semantics", "Objects and arrays preserve containment, order, and one-to-many structure. Flattening nested values into columns or key-value rows can lose the distinction between elements that belonged together.", "Arrays are values, not hidden child tables.", "Repeated field names remain scoped by their path.", "Absent, null, and empty are separate states."),
            chapter("JSONB and BSON optimize different boundaries", "PostgreSQL JSONB stores a binary representation inside a relational tuple while MongoDB BSON is native to the document engine and wire protocol. Both expose typed values, but storage and execution paths differ.", "JSONB participates in tuple visibility and TOAST.", "BSON includes document-oriented scalar types.", "Wire format, in-memory format, and durable format need not match."),
            chapter("Indexes select paths and array semantics", "A document index must identify paths, value types, and whether array elements produce multiple keys. General inverted indexes broaden coverage while targeted indexes preserve ordering and selectivity.", "Compound key order still governs equality, sort, and range.", "Multikey expansion affects coverage and key combinations.", "An index on a document is not an index for every expression."),
            chapter("Large updates amplify physical work", "Changing one field can rewrite a large JSONB tuple, create new MVCC versions, and prevent HOT updates when indexed expressions change. Native document engines also pay allocation and index-maintenance costs.", "Measure bytes written per logical field update.", "Separate frequently changing values from cold large payloads.", "Compression can reduce storage while increasing rewrite CPU."),
            chapter("Cross-document rules need explicit coordination", "Single-document atomicity makes embedded invariants straightforward. Constraints spanning documents require transactions, unique indexes, validation, or application protocols with clearly stated isolation assumptions.", "Denormalization duplicates both data and repair obligations.", "Write skew survives when concurrent transactions protect different records.", "Foreign-key absence does not remove referential semantics."),
        ],
        "sources": ["Intro to PostgreSQL with JSONB compared to MongoDB, a general purpose document database", "BSON and OSON: documents are designed to be nested, not flat", "Querying embedded arrays in JSON (PostgreSQL JSONB and MongoDB documents)", "PostgreSQL JSONB Indexing Limitations with B-Tree and GIN", "JSONB DeTOASTing (read amplification) when using PostgreSQL as a Document Database", "No HOT updates on JSONB (write amplification) performance impact", "Document data modeling to avoid write skew anomalies", "Where SQL joins struggle but MongoDB documents shine"],
    },
    {
        "slug": "database-connections",
        "number": "24",
        "title": "Database Connections",
        "subtitle": "Pools, sessions, admission, and failure diagnosis",
        "description": "Treat database connections as bounded stateful resources, from listener and authentication handshakes to pooling, serverless bursts, and session cleanup.",
        "accent": "#9a4f47",
        "topics": "connections · pooling · sessions · admission",
        "chapters": [
            chapter("A connection is a protocol lifecycle", "Opening a database connection traverses naming, networking, transport security, authentication, and session initialization. An error at each layer needs different evidence.", "Resolve the endpoint before debugging credentials.", "Distinguish connect timeout from query timeout.", "Retain server and client error details without exposing secrets."),
            chapter("A session consumes server state", "A connected client may own a process, memory, transaction context, prepared statements, temporary objects, and observability identity. Idle does not mean free.", "Capacity planning starts with concurrent active work.", "Idle-in-transaction sessions retain locks and versions.", "Session initialization must be repeatable."),
            chapter("Pools trade setup cost for state hygiene", "A pool reuses established connections and queues demand above its size. It improves admission control only when checkout deadlines, validation, reset behavior, and transaction boundaries are correct.", "Size pools from database capacity across all application instances.", "Always return connections after rollback or commit.", "Reset tenant, role, schema, and tracing state before reuse."),
            chapter("Multiplexing changes session assumptions", "Transaction pooling and database-resident connection managers can route successive transactions through different server processes. Features tied to one backend may no longer persist across calls.", "Prepared statement support depends on pool mode.", "Temporary tables and session variables can pin sessions.", "Observe logical clients separately from physical backends."),
            chapter("Serverless bursts need admission control", "Functions can scale clients faster than a database can create sessions. A bounded intermediary, concurrency limit, and backpressure protect the database better than unlimited connection retries.", "Reuse clients across warm invocations.", "Add jitter to bounded retries.", "Budget connections across versions, regions, and deployment overlap."),
            chapter("Diagnose from both endpoints", "Client exceptions show the attempted endpoint and phase; server listeners, authentication logs, and session views show what arrived. Correlating timestamps and request identity separates refusal, timeout, and post-connect failure.", "Test from the same network path as the application.", "Count connection rate as well as concurrent sessions.", "Do not diagnose intermittent exhaustion with one successful login."),
        ],
        "sources": ["YugabyteDB Connection Manager: a Database Resident Connection Pool with Shared Processes", "The Complete Guide to Troubleshooting Oracle Connection Errors", "Serverless DB connections: Lambda to YugabyteDB 🚀", "Oracle: connection rate to the listener", "Easy Oracle Cloud wallet location in the JDBC connection string"],
    },
    {
        "slug": "vector-search",
        "number": "25",
        "title": "Vector Search",
        "subtitle": "Embeddings, ANN indexes, filters, and hybrid retrieval",
        "description": "Build and evaluate semantic retrieval with vector representations, approximate indexes, metadata filtering, and full-text hybrid ranking.",
        "accent": "#397181",
        "topics": "embeddings · ANN · HNSW · hybrid search",
        "chapters": [
            chapter("Embeddings turn meaning into geometry", "An embedding model maps content into fixed-length vectors where a distance function approximates semantic similarity. Query and corpus vectors must share the same model and normalization contract.", "Model version is part of stored-data provenance.", "Cosine, inner product, and Euclidean distance are not interchangeable by accident.", "Chunking determines what one result can mean."),
            chapter("Exact search defines the baseline", "Brute-force distance over every eligible vector provides a correctness reference. Approximate nearest-neighbor indexes reduce latency by exploring only a promising subset.", "Measure ANN recall against exact top-k results.", "Dataset size and dimensionality shape the crossover point.", "Index build time and memory belong in the cost model."),
            chapter("ANN indexes expose quality controls", "Graph and partition-based indexes tune search breadth, construction effort, memory, and recall. Higher search effort usually improves quality while increasing latency.", "HNSW navigates a layered proximity graph.", "DiskANN-style designs optimize graph access for secondary storage.", "Defaults are starting points, not workload guarantees."),
            chapter("Filtering changes the search space", "Metadata predicates can run before, during, or after ANN traversal. Post-filtering may return fewer than k rows and reduce recall when many visited candidates are ineligible.", "Evaluate recall after applying the business filter.", "Selective filters need index-aware traversal or oversampling.", "Partitioning by metadata can improve one filter and fragment others."),
            chapter("Hybrid search combines distinct evidence", "Full-text ranking captures lexical precision while vector similarity captures semantic proximity. Hybrid retrieval normalizes and fuses ranks rather than pretending both scores share one scale.", "Reciprocal-rank fusion is robust without score calibration.", "Retain component ranks for explanation and tuning.", "Candidate generation and reranking are separate stages."),
            chapter("Evaluation starts with relevant judgments", "Latency and throughput cannot establish retrieval quality. A representative query set with expected relevant documents supports recall, precision, ranking, and regression measurements.", "Measure at the final requested k.", "Include rare filters and difficult negatives.", "Re-embed or version indexes when the model changes."),
        ],
        "sources": ["MongoDB Vector Search Index, With Local Atlas and Ollama", "No pre-filtering in pgvector means reduced ANN recall (impact on Vector Search accuracy)", "Vector Search with Filters: pgvector vs DiskANN on HorizonDB", "Hybrid Search (Full-Text and Vector Similarity) in HorizonDB", "Embedding Into JSONB Still Feels Like a JOIN for Large Documents"],
    },
]
...
EXTRA_TECHNICAL_GUIDES = {
    ...
    "document-data-modeling": [
        guide("Choose an aggregate boundary", "Write down which values must be atomically consistent, loaded together, and deleted together. Embed that owned state; reference independently shared or unbounded state.", "Order {\n  _id, customerId, status,\n  lines: [{ productId, quantity, priceAtOrder }],\n  shippingAddress: { ... }\n}\n// Product catalog and customer remain referenced aggregates."),
        guide("Index a concrete JSON path", "In PostgreSQL, a targeted expression index supports a known scalar predicate and ordering more efficiently than assuming one broad GIN index covers every operation.", "CREATE INDEX orders_customer_created_idx\nON orders ((payload->>'customerId'), created_at DESC);\n\nEXPLAIN (ANALYZE, BUFFERS)\nSELECT * FROM orders\nWHERE payload->>'customerId' = $1\nORDER BY created_at DESC LIMIT 20;"),
        guide("Preserve same-element array meaning", "When multiple predicates must match one array element, keep them in one element expression. Independent path predicates can accidentally match values from different elements.", "db.orders.find({\n  lines: { $elemMatch: {\n    productId: 'P42', quantity: { $gte: 10 }\n  }}\n})"),
        guide("Measure update amplification", "Compare one logical field change with tuple versions, WAL, table growth, and index writes. Large hot JSON payloads often deserve decomposition even when reads remain document-shaped.", "EXPLAIN (ANALYZE, WAL, BUFFERS)\nUPDATE profile\nSET document = jsonb_set(document, '{lastSeen}', to_jsonb(now()))\nWHERE id = $1;\n-- Check n_tup_hot_upd, WAL bytes, and relation growth over a workload."),
    ],
    "database-connections": [
        guide("Budget the global pool", "A per-instance pool limit multiplies by replicas, workers, regions, and overlapping deployments. Reserve capacity for administration and background jobs before dividing the remainder.", "usable_connections = database_limit - admin_reserve - job_reserve\nper_instance_pool = usable_connections // maximum_app_instances\n# Include blue/green overlap and autoscaling maximums."),
        guide("Set bounded pool behavior", "Checkout timeout limits queueing, connection timeout limits establishment, and statement timeout limits execution. They protect different phases and should produce distinguishable telemetry.", "pool.max_size = 20\npool.checkout_timeout = '2s'\nconnect_timeout = '5s'\nstatement_timeout = '10s'\n# Retry only operations whose outcome is known or idempotent."),
        guide("Find PostgreSQL connection pressure", "Group sessions by application, state, and transaction age. A pool with many idle backends may be oversized; idle-in-transaction sessions are correctness and cleanup defects.", "SELECT application_name, state, count(*),\n       max(now()-xact_start) AS oldest_transaction\nFROM pg_stat_activity\nWHERE backend_type = 'client backend'\nGROUP BY application_name, state\nORDER BY count(*) DESC;"),
        guide("Classify a failed connection", "Capture endpoint, DNS result, route, TLS phase, authentication result, and server log timestamp in order. This prevents a generic connection error from collapsing unrelated failures.", "1. Resolve host and selected address.\n2. Test TCP reachability from the application network.\n3. Verify TLS name, trust, and wallet/certificate expiry.\n4. Correlate listener and authentication logs.\n5. Check session limits and pool saturation."),
    ],
    "vector-search": [
        guide("Create an exact quality baseline", "Keep a representative query set and compute exact nearest neighbors before tuning ANN. The baseline lets recall changes be measured rather than inferred from plausible-looking results.", "SELECT id, embedding <=> $1 AS cosine_distance\nFROM article\nWHERE tenant_id = $2\nORDER BY embedding <=> $1\nLIMIT 10;\n-- Run without an ANN path for the labeled evaluation sample."),
        guide("Measure recall at k", "Recall compares relevant exact neighbors recovered by ANN. Report its distribution across queries and filters alongside latency percentiles.", "recall_at_k = len(set(exact_top_k) & set(ann_top_k)) / k\n# Report mean, p5, and worst cohorts by filter selectivity."),
        guide("Test filtering explicitly", "A selective predicate can starve post-filtered ANN results. Compare returned row count and recall while increasing traversal effort or using an index that incorporates filtering.", "SET LOCAL hnsw.ef_search = 100;\nSELECT id FROM article\nWHERE category = $1\nORDER BY embedding <=> $2\nLIMIT 20;\n-- Sweep ef_search and category selectivity; record recall and latency."),
        guide("Fuse lexical and semantic ranks", "Reciprocal-rank fusion combines independent ranked lists without assuming text and vector scores have comparable scales.", "rrf_score = 1 / (60 + text_rank) + 1 / (60 + vector_rank)\n# Union candidate IDs, assign missing ranks no contribution,\n# sort by rrf_score, and retain both ranks for diagnosis."),
    ],
}
